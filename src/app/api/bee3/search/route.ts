...
export async function GET(request: NextRequest) {
    ...
    if (!categoryPath && !search) return NextResponse.json({ error: "invalid-query" }, { status: 400 });

    const pageNum = +(request.nextUrl.searchParams.get("page") ?? 0);
    const pageSize = Math.min(Math.max(+(request.nextUrl.searchParams.get("pageSize") ?? DEFAULT_PAGE_SIZE), MIN_PAGE_SIZE), MAX_PAGE_SIZE);
    ...
    let order = Prisma.SortOrder[request.nextUrl.searchParams.get("order") as keyof typeof Prisma.SortOrder]; // asc, desc
    if (!order) order = Prisma.SortOrder.desc;

    const paths = categoryPath ? [categoryPath].concat(getSubCategoryPaths(categoryPath)) : [];
    ...
    const adsPromise = db.ad.findMany({
        ...
        where: {
            ...
            price: price ? { gte: +price.split('-')[0]!, lte: +price.split('-')[1]! } : undefined
        },
